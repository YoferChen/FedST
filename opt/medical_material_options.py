...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--name', type=str, default='medical_material', required=False, help='name of the experiment.')
        parser.add_argument('--model', type=str, default='unet', choices=['unet', 'fedst', 'fedst_ddpm'],
                            help='chooses which model to use,unet...')
        parser.add_argument('--federated_algorithm', type=str, default='fedavg',
                            choices=['fedavg', 'fedprox', 'feddyn', 'feddc', 'fedddpm'],
                            help='chooses which federated learning algrithom to use')
        parser.add_argument('--net', type=str, default='unet', choices=['unet'],
                            help='chooses which network to use,unet...')
        ...
        parser.add_argument('--log_dir', default='fedst.log', type=str,
        ...
        parser.add_argument('--dataroot', default='./dataset/medical_material', help='path to images')
        parser.add_argument('--resultroot', default='./ckpt_medical', help='path to ckpt')
        parser.add_argument('--data_type', default='splited', help='[pure|mix|splited]')

        parser.add_argument('--num_threads', default=4, type=int, help='# threads for loading data')
        parser.add_argument('--serial_batches', default=False, action='store_true',
        ...
        parser.add_argument('--dataset_mode', type=str, default='aligned',
                            help='chooses how datasets are loaded. [aligned,gan]')
        ...
        # loss
        parser.add_argument('--loss_type', choices=['CrossEntropyLoss', 'BCW', 'Focal'], default='Focal',
                            help='loss types')
...
class TrainOptions(BaseOptions):
    def initialize(self, parser):
        ...
        parser.add_argument('--cross_validation', type=bool, default=True,
                            help='if using cross validation to choose the model')
        parser.add_argument('--folds', type=int, default=5, help='how many folds in cross_validation')
        parser.add_argument('--total_img_dir_real', type=str, default='train/real_image',
                            help='where are the training images')
        parser.add_argument('--total_label_dir_real', type=str, default='train/real_label',
                            help='where are the training images')
        ...
        parser.add_argument('--lr_gamma', type=float, default=0.927,
