...
class ResBlock(EmbedBlock):
    ...
    def __init__(
        ...
        if use_style:
            self.style = StyleMod(dlatent_size, channels=self.out_channel, use_wscale=use_wscale)
    ...
    def forward(self, x, emb, latent):
        ...
        """
        return checkpoint(
            self._forward, (x, emb, latent), self.parameters(), self.use_checkpoint
        )

    def _forward(self, x, emb, latent):
        if self.updown:
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.class_embedding = nn.Sequential(*[nn.Linear(self.num_client, latent_size)])
    ...
    def forward(self, x, gammas, latent):
        """
        ...
        output = torch.zeros(len(latent), self.num_client).to(latent.device)
        # self.class_embedding = self.class_embedding.to(class_.device)
        class_onehot = output.scatter(1, latent.unsqueeze(-1).type(torch.int64), 1)
        style_embedding = self.class_embedding(class_onehot)

        h = x.type(torch.float32)
        for module in self.input_blocks:
            h = module(h, emb, style_embedding)
            hs.append(h)
        h = self.middle_block(h, emb, style_embedding)
