...
def parse(args):
    ...
    if args.client is not None:
        opt['datasets'][opt['phase']]['which_dataset']['args']['data_client'] = args.client
        opt['datasets'][opt['phase']]['which_dataset']['args']['data_flist'] = opt['datasets'][opt['phase']]['which_dataset']['args']['data_flist'].split('.flist')[0]+f'.flist{args.client}'
    ...
    if opt['phase'] == 'test':
        data_client = opt['datasets'][opt['phase']]['which_dataset']['args']['data_client']
        experiments_root = os.path.join(opt['path']['base_dir'],
                                        '{}_data{}_{}'.format(opt['name'], data_client, get_timestamp()))
