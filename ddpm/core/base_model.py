...
class BaseModel():
    ...
    def load_network(self, network, network_label, strict=True):
        ...
        for i in keys:
            ...
            #     key = '.'.join(i.split('.')[1:])
            #     sd[key] = state_dict[i]
            # elif 'denoise_fn' in i :
            #     if self.phase == 'train':
            #         key = '.'.join(i.split('.')[1:])   # 训练时
            #     else:
            #         key = i  # 推理时
            #     sd[key] = state_dict[i]
            sd[i] = state_dict[i]
        ...
        print(f'Loading pretrained model finished! Num layers: {count}')
        self.logger.info(f'Loading pretrained model finished! Num layers: {count}')
