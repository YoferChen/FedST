...
def create_test_configs():
    ...
    AAF_separate_config_dir = os.path.join(config_dir, 'AAFnew_separate_test')
    ...
    for style_id in tqdm(range(79)):
        ...
        for data_id in data_source_id:
            ...
            # 风格模型
            opt['path'][
                'resume_state'] = f"experiments/{model_dict[style_id]['name']}/checkpoint/{model_dict[style_id]['epoch']}"
    ...
    for task_id in range(task_num):
        ...
        with open(sh_path, 'w', encoding='utf-8') as f:
            ...
            for t in range(total_num, min(total_num + task_num_for_each_gpu, total)):
                ...
                config_base_dir = os.path.join(base_dir, 'config', 'AAFnew_separate_test')
                json_list = sorted(glob(os.path.join(config_base_dir, str(t), '*')))
                for json_path in json_list:
                    json_name = os.path.basename(json_path)
                    f.write(
                        f"python ./run.py -c config/AAFnew_separate_test/{t}/{json_name} -p test --gpu_ids {task_id}\n")
        ...
        with open(sh_path, 'w', encoding='utf-8') as f:
            ...
            for age in range(total_num, min(total_num + task_num_for_each_gpu, total)):
                ...
                for fake_age in fake_ages:
                    ...
                    f.write(
                        f"python ./run.py -c config/AAFnew_separate_test/{fake_age}/{json_name} -p test --gpu_ids {task_id}\n")
            f.write('echo "Scripts have finished running."')
        total_num += task_num_for_each_gpu
