...
class Palette(BaseModel):
    def __init__(self, networks, losses, sample_num, task, optimizers, ema_scheduler=None, **kwargs):
        ...
        # scheduler = lr_scheduler.LambdaLR(self.optG, one_cycle(1, 0.0001, self.opt['train']['n_epoch']))
        # scheduler1 = StepLR(self.optG, step_size=10, gamma=0.5)
        # self.schedulers.append(scheduler1)
    ...
    def set_input(self, data):
        ...
        self.client_class = self.set_device(data.get('class'))
        self.model_class = self.set_device(data.get('model_class'))
    ...
    def train_step(self):
        ...
        for train_data in tqdm.tqdm(self.phase_loader):
            ...
            self.optG.zero_grad()
            loss = self.netG(self.gt_image, self.cond_image, mask=self.mask, class_=self.client_class)
            loss.backward()
            self.optG.step()
        ...
        for scheduler in self.schedulers:
            scheduler.step()
        # self.writer.add_scalar("learning_rate", self.schedulers[0].get_last_lr()[0])
                ...
                if self.opt['distributed']:
                    if self.task in ['inpainting','uncropping']:
                        self.output, self.visuals = self.netG.module.restoration(self.cond_image, y_t=self.cond_image, 
                            y_0=self.gt_image, mask=self.mask, sample_num=self.sample_num, class_=self.client_class)
                    else:
                        self.output, self.visuals = self.netG.module.restoration(self.cond_image, sample_num=self.sample_num, class_=self.client_class)
                else:
                    if self.task in ['inpainting','uncropping']:
                        self.output, self.visuals = self.netG.restoration(self.cond_image, y_t=self.cond_image, 
                            y_0=self.gt_image, mask=self.mask, sample_num=self.sample_num, class_=self.client_class)
                    else:
                        self.output, self.visuals = self.netG.restoration(self.cond_image, sample_num=self.sample_num, class_=self.client_class)
        ...
        with torch.no_grad():
            for phase_data in tqdm.tqdm(self.phase_loader):
                ...
                if self.opt['distributed']:
                    if self.task in ['inpainting','uncropping']:
                        self.output, self.visuals = self.netG.module.restoration(self.cond_image, y_t=self.cond_image, 
                            y_0=self.gt_image, mask=self.mask, sample_num=self.sample_num, class_=self.model_class)
                    else:
                        self.output, self.visuals = self.netG.module.restoration(self.cond_image, sample_num=self.sample_num, class_=self.model_class)
                else:
                    if self.task in ['inpainting','uncropping']:
                        self.output, self.visuals = self.netG.restoration(self.cond_image, y_t=self.cond_image, 
                            y_0=self.gt_image, mask=self.mask, sample_num=self.sample_num, class_=self.model_class)
                    else:
                        self.output, self.visuals = self.netG.restoration(self.cond_image, sample_num=self.sample_num, class_=self.model_class)
